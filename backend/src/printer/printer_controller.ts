...
import { VentasService } from "../services/ventas_service"; // ✅ corregido
...
export const printTicket = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!id_venta) return res.status(400).json({ ok: false, error: "id_venta requerido" });
    ...
    const items = (venta.registroVentas ?? []).map((rv: any) => {
      ...
      const precio =
        Number(rv.producto?.precio_venta ?? 0) || (cantidad > 0 ? Math.round(subtotal / cantidad) : 0);
    ...
    await imprimirTicket({
      ...
      resumen,
      conIva: conIvaFinal,
      tipo_documento: (venta as any).tipo_documento ?? "boleta",
    } as any);
