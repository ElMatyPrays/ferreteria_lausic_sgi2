...
const printTicket = async (req, res) => {
    ...
    try {
        ...
        const items = (venta.registroVentas ?? []).map((rv) => {
            ...
            const precio = Number(rv.producto?.precio_venta ?? 0) ||
        ...
        const cliente = venta.tipo_documento === "factura" && venta.cliente
            ? {
                rut: String(venta.cliente.rut ?? ""),
                razon_social: String(venta.cliente.razon_social ?? ""),
                giro: String(venta.cliente.giro ?? ""),
                direccion: String(venta.cliente.direccion ?? ""),
                comuna: String(venta.cliente.comuna ?? ""),
                ciudad: String(venta.cliente.ciudad ?? ""),
                contacto: String(venta.cliente.contacto ?? ""),
            }
            : undefined;
