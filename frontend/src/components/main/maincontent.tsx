...
import {
  buildVentasColumns,
} from "../main/secciones/ventas/ventas.columns";
...
const columnsBySection: Record<Section, Column[]> = {
  Productos: [
    { key: "SKU", header: "SKU", width: "50px" },
    { key: "codigo_barras", header: "Código de barras", width: "120px" },
    { key: "nombre", header: "Nombre", width: "180px" },
    { key: "tipo", header: "Tipo", width: "120px" },
    { key: "proveedor", header: "Proveedor", width: "160px" },
    { key: "precio_compra", header: "Precio Compra", width: "70px", align: "right", numeric: true },
    { key: "stock", header: "Stock", width: "50px", align: "right", numeric: true },
    { key: "unidad_medida", header: "Unidad de Medida", width: "100px", align: "right" },
    { key: "precio_venta", header: "Precio Venta", width: "60px", align: "right", numeric: true },
  ],
  Ventas: [],
  Clientes: [
    { key: "rut", header: "RUT" },
    { key: "razon_social", header: "Razón social" },
    { key: "tipo_de_compra", header: "Tipo de compra" },
...
export default function MainContent({ section, venTab: venTabProp, onVenTabChange }: MainContentProps) {
  ...
  const canCreate = isAdmin || (isOperador && section === "Productos"); // ✅ operador crea en Productos
  const canEdit = isAdmin;                                             // (deja edit solo admin)
  ...
  if (section === "Ventas") {
    // 🔥 SIEMPRE generar columnas nuevas
    return buildVentasColumns(venTab);
  }

  // 🔒 copia defensiva para evitar referencias congeladas
  return columnsBySection[section].map((c) => ({ ...c }));
}, [section, venTab]);
  ...
  const isReadOnly = isReader; // ✅ solo LECTOR es read-only total
  ...
  if (active.key !== "demo") {
    headerRight = (
      <div className="mc-actions">
        ...
        {active.create && canCreate && active.key !== "ventas" && active.key !== "lista_ventas" && (
  ...
  return (
    <section className="mc-root" aria-label={`Tabla de ${section}`}>
      ...
      <DataTable
        ...
        onEdit={isReadOnly || !canEdit || active.key === "lista_ventas" ? undefined : openEdit}
